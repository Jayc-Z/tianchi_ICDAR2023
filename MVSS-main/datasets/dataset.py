...
class Edge_generator(torch.nn.Module):
    """generate the 'edge bar' for efficientnet_b3_img512512_bs8 0-1 mask Groundtruth of efficientnet_b3_img512512_bs8 image
    Algorithm is based on 'Morphological Dilation and Difference Reduction'
    
    Which implemented with fixed-weight Convolution layer with weight matrix looks like efficientnet_b3_img512512_bs8 cross,
...
class DeepfakeDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if (mask_file_name == "None"):
            mask = np.zeros((height, width), np.uint8) # efficientnet_b3_img512512_bs8 totally black mask for real image
        else:
        ...
        if (edge_file_name == "None"):
            edge = np.zeros((height, width), np.uint8) # efficientnet_b3_img512512_bs8 totally black edge mask for real image
        else:
        ...
        # ----------
        seed = np.random.randint(2147483647) # make efficientnet_b3_img512512_bs8 seed with numpy generator
