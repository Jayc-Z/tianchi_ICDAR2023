...
from se_resnext import build_resnext_101
...
def train_one_epoch(model, train_loader, optimizer, losses_dict):
    ...
    for _, (images, gt) in pbar:
        ...
        with amp.autocast(enabled=True):
            ...
            # losses = (ce_loss + focalloss + 2 * weight_bceloss) / 4
            losses = focalloss
...
def main():
    ...
    if config.TRAIN_VALID_FLAG:
        ...
        for fold in range(config.N_FOLD):
            ...
            train_loader, valid_loader = build_dataloader(df, config.TRAIN_VALID_FLAG, fold, data_transforms) # dataset & dtaloader
            # model = build_model(pretrain_flag=True)
            model = build_resnext_101()
